...
fn calculate_density<'a, T>(smooth_radius: f32, sample_pos: Vec2, positions: &T) -> f32
    where T: IntoIterator<Item=&'a Vec2> + Clone
{
    let mut density = 0.;
    for &position in positions.clone() {
        let dst = (position - sample_pos).length();
...
pub fn calculate_gradient<'a, T>(
    smooth_radius: f32,
    sample_pos: Vec2, positions: &T
) -> Vec2
    where T: IntoIterator<Item=&'a Vec2> + Clone
{
    let step_size = 0.001;
